...
class GaussianPDFFit : public ROOT::Minuit2::FCNBase {
  public:
    ...
    int nparams =5; 
     ...
     GaussianPDFFit(ComboHitCollection _chits,StrawResponse _srep, CosmicTrack _track, std::vector<double> &_constraint_means, std::vector<double> &_constraints, double _sigma_t, int _k) :  chits(_chits),  srep(_srep), track(_track), constraint_means(_constraint_means), constraints(_constraints) , sigma_t(_sigma_t), k(_k) {};
   
    double Up() const { return 0.5; };
    double operator() (const std::vector<double> &x) const;
    double TimeResidual(double doca, StrawResponse srep, double t0, ComboHit hit) const ;
...
class FullDriftFit : public GaussianPDFFit {
  public:
    FullDriftFit(ComboHitCollection _chits,  StrawResponse _srep, CosmicTrack _track, std::vector<double> &_constraint_means, std::vector<double> &_constraints, double sigma_t, int _k);
int Factorial(int k);
