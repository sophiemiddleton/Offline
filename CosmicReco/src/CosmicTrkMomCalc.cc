...
//------------------------------------------------------------------------
CosmicTrkMomCalc::~CosmicTrkMomCalc() {
//------------------------------------------------------------------------
}
...
//------------------------------------------------------------------------
double
CosmicTrkMomCalc::ptMom(const TrkSimpTraj& theTraj, const BField&
...
//------------------------------------------------------------------------
Hep3Vector
CosmicTrkMomCalc::vecMom(const TrkSimpTraj& theTraj, const BField&
                         theField, double fltlen) {
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
BbrVectorErr
CosmicTrkMomCalc::errMom(const TrkSimpTraj& theTraj, const BField&
                         theField, double fltlen) {
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
int
CosmicTrkMomCalc::charge(const TrkSimpTraj& theTraj, const BField&
                         theField, double fltlen) {
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
HepMatrix
CosmicTrkMomCalc::posmomCov(const TrkSimpTraj& theTraj,const BField& theField,
			    double fltlen) {
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
void
CosmicTrkMomCalc::getAllCovs(const TrkSimpTraj& theTraj,
...
//------------------------------------------------------------------------
void
CosmicTrkMomCalc::getAllWeights(const TrkSimpTraj& theTraj,
				const BField& theField,
				double fltlen,
				HepVector& pos,
				HepVector& mom,
...
//------------------------------------------------------------------------
Hep3Vector
CosmicTrkMomCalc::calcCurvVecMom(const Hep3Vector& direction, 
                                 double curvature,
                                 const BField& theField) {
//------------------------------------------------------------------------
...
DifNumber
CosmicTrkMomCalc::momMag(const HelixTraj& theTraj,
			 const BField& theField) {
...
//------------------------------------------------------------------------
BbrVectorErr
CosmicTrkMomCalc::calcCurvErrMom(const TrkSimpTraj& theTraj,
                                 const BField& theField,
                                 double fltlen) {
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
BbrVectorErr
CosmicTrkMomCalc::calcNeutErrMom(const TrkSimpTraj& theTraj,
                                 const BField& theField,
                                 double fltlen) {
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
int
CosmicTrkMomCalc::calcCurvCharge(const Hep3Vector& direction,
                                 double curvature,
                                 const BField& theField) {
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
int
CosmicTrkMomCalc::nearestInt(double floatPt) {
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------ 
HepMatrix
CosmicTrkMomCalc::calcCurvPosmomCov(const TrkSimpTraj& theTraj,
				    const BField& theField,
				    double fltlen) {
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
HepMatrix
CosmicTrkMomCalc::calcNeutPosmomCov(const TrkSimpTraj& theTraj,
				    const BField& theField,
				    double fltlen) {
//------------------------------------------------------------------------
...
bool CosmicTrkMomCalc::weightToCov(const HepSymMatrix& inXX,const HepSymMatrix& inPP,const HepMatrix& inXP,
...
//------------------------------------------------------------------------
void
CosmicTrkMomCalc::calcCurvAllCovs(const TrkSimpTraj& theTraj,
				  const BField& theField,
				  double fltlen,
				  HepSymMatrix& xxCov,
				  HepSymMatrix& ppCov,
...
//------------------------------------------------------------------------
void
CosmicTrkMomCalc::calcCurvAllCovsOLD(const TrkSimpTraj& theTraj,
				  const BField& theField,
				  double fltlen,
				  HepSymMatrix& xxCov,
				  HepSymMatrix& ppCov,
...
//------------------------------------------------------------------------
void
CosmicTrkMomCalc::calcNeutAllCovs(const TrkSimpTraj& theTraj,
				  const BField& theField,
				  double fltlen,
				  HepSymMatrix& xxCov,
				  HepSymMatrix& ppCov,
...
//------------------------------------------------------------------------
  ...
  double pt = theTraj.mom()*sintheta;
  //px= pt*cosphi*sintheta, py=pt*sintheta*sinphi, pz=pt*costheta

  // Calculate derivatives for Jacobian matrix //
  double d_x_d0 = -1*sinphi0;
  double d_x_phi0 = -d0*cosphi0;
  double d_x_theta = fltlen*cosphi*costheta;
  double d_x_phi = -1*fltlen*sinphi*sintheta;

  double d_y_d0 = cosphi0;
  double d_y_phi0 = -1*d0*sinphi0;
  ...
  double m_d0_theta =  m[CosmicLineParams::d0Index][CosmicLineParams::phiIndex];
  double m_d0_phi =  m[CosmicLineParams::d0Index][CosmicLineParams::phiIndex];

  double m_phi0_phi0 =  m[CosmicLineParams::phi0Index][CosmicLineParams::phi0Index];
  double m_phi0_phi =  m[CosmicLineParams::phi0Index][CosmicLineParams::phiIndex];
  ...
  double m_phi_theta =  m[CosmicLineParams::phiIndex][CosmicLineParams::thetaIndex];
  double m_phi_phi =  m[CosmicLineParams::phiIndex][CosmicLineParams::phiIndex];
  
  xxCov(1,1) = 
    d_x_d0* (  d_x_d0*m_d0_d0 + d_x_phi0*m_phi0_d0 + d_x_theta*m_theta_d0 + d_x_phi*m_phi_d0  )   + 
    d_x_phi0* (  d_x_d0*m_d0_phi0 + d_x_phi0*m_phi0_phi0 + d_x_theta*m_theta_phi0 + d_x_phi*m_phi_phi0  )   + 
  ...
  xxCov(2,1) = 
    d_y_d0* (  d_x_d0*m_d0_d0 + d_x_phi0*m_d0_phi0 + d_x_theta*m_theta_d0 + d_x_phi*m_phi_d0  )   + 
    d_y_phi0* (  d_x_d0*m_d0_phi0 + d_x_phi0*m_phi0_phi0 + d_x_theta*m_theta_phi0 + d_x_phi*m_phi_phi0  )   + 
  ...
  xxCov(2,2) = 
    d_y_d0* (  d_y_d0*m_d0_d0 + d_y_phi0*m_d0_phi0 + d_y_theta*m_theta_d0 + d_y_phi*m_phi_d0  )   + 
    d_y_phi0* (  d_y_d0*m_d0_phi0 + d_y_phi0*m_phi0_phi0 + d_y_theta*m_theta_phi0 + d_y_phi*m_phi_phi0  )   + 
  ...
  xxCov(3,1) =  
    dz_d0*(  d_x_d0*m_d0_d0 + d_x_phi0*m_d0_theta + d_x_theta*m_d0_theta + d_x_phi*m_d0_theta )+
    dz_dphi0*(  d_x_d0*m_d0_phi0 + d_x_phi0*m_phi0_phi0 + d_x_theta*m_phi0_theta + d_x_phi*m_phi0_phi );  
    d_z_theta*(  d_x_d0*m_d0_theta + d_x_phi0*m_phi0_theta + d_x_theta*m_theta_theta + d_x_phi*m_phi_theta )+
   dz_phi*(  d_x_d0*m_d0_pho + d_x_phi0*m_phi0_phi + d_x_theta*m_phi_theta + d_x_phi*m_phi_phi );  

  xxCov(3,2) = dz_d0*(  d_y_d0*m_d0_d0 + d_y_phi0*m_d0_theta + d_y_theta*m_d0_theta + d_y_phi*m_d0_theta )+
    dz_dphi0*(  d_y_d0*m_d0_phi0 + d_y_phi0*m_phi0_phi0 + d_y_theta*m_phi0_theta + d_y_phi*m_phi0_phi );  
    d_z_theta*(  d_y_d0*m_d0_theta + d_y_phi0*m_phi0_theta + d_y_theta*m_theta_theta + d_y_phi*m_phi_theta )+
   dz_phi*(  d_y_d0*m_d0_pho + d_y_phi0*m_phi0_phi + d_y_theta*m_phi_theta + d_y_phi*m_phi_phi );  

  xxCov(3,3) = dz_d0*(  d_z_d0*m_d0_d0 + d_z_phi0*m_d0_theta + d_z_theta*m_d0_theta + d_z_phi*m_d0_theta )+
    dz_dphi0*(  d_z_d0*m_d0_phi0 + d_z_phi0*m_phi0_phi0 + d_z_theta*m_phi0_theta + d_z_phi*m_phi0_phi );  
    d_z_theta*(  d_z_d0*m_d0_theta + d_z_phi0*m_phi0_theta + d_z_theta*m_theta_theta + d_z_phi*m_phi_theta )+
   dz_phi*(  d_z_d0*m_d0_pho + d_z_phi0*m_phi0_phi + d_z_theta*m_phi_theta + d_z_phi*m_phi_phi );  


  xpCov(1,1) = d_px_theta* (  d_x_d0*m_theta_d0 + d_x_phi0*m_theta_phi0 + d_x_theta*m_theta_theta + d_x_phi*m_phi_theta  )   + 
    d_px_phi* (  d_x_d0*m_phi_d0 + d_x_phi0*m_phi_phi0 + d_x_theta*m_phi_theta + d_x_phi*m_phi_phi  )   ; 

  xpCov(2,1) = d_px_theta* (  d_y_d0*m_theta_d0 + d_y_phi0*m_theta_phi0 + d_y_theta*m_theta_theta + d_y_phi*m_phi_theta  )   + 
    d_px_phi* (  d_y_d0*m_phi_d0 + d_y_phi0*m_phi_phi0 + d_y_theta*m_phi_theta + d_y_phi*m_phi_phi  )   ; 

  xpCov(3,1) = d_px_theta* (  d_z_d0*m_theta_d0 + d_z_phi0*m_theta_phi0 + d_z_theta*m_theta_theta + d_z_phi*m_phi_theta  )   + 
    d_px_phi* (  d_z_d0*m_phi_d0 + d_z_phi0*m_phi_phi0 + d_z_theta*m_phi_theta + d_z_phi*m_phi_phi  )   ;

  xpCov(1,2) =  d_py_theta* (  d_x_d0*m_theta_d0 + d_x_phi0*m_theta_phi0 + d_x_theta*m_theta_theta + d_x_phi*m_phi_theta  )   + 
    d_py_phi* (  d_x_d0*m_phi_d0 + d_x_phi0*m_phi_phi0 + d_x_theta*m_phi_theta + d_x_phi*m_phi_phi  )   ;

  xpCov(2,2) =  d_py_theta* (  d_y_d0*m_theta_d0 + d_y_phi0*m_theta_phi0 + d_y_theta*m_theta_theta + d_y_phi*m_phi_theta  )   + 
    d_py_phi* (  d_y_d0*m_phi_d0 + d_y_phi0*m_phi_phi0 + d_y_theta*m_phi_theta + d_y_phi*m_phi_phi  )   ;
    
  xpCov(3,2) =  d_py_theta* (  d_z_d0*m_theta_d0 + d_z_phi0*m_theta_phi0 + d_z_theta*m_theta_theta + d_y_phi*m_phi_theta  )   + 
    d_py_phi* (  d_z_d0*m_phi_d0 + d_z_phi0*m_phi_phi0 + d_z_theta*m_phi_theta + d_z_phi*m_phi_phi  )   ;
    
  xpCov(1,3) =  d_pz_theta* (  d_x_d0*m_theta_d0 + d_x_phi0*m_theta_phi0 + d_x_theta*m_theta_theta + d_x_phi*m_phi_theta  )   + 
    d_pz_phi* (  d_x_d0*m_phi_d0 + d_x_phi0*m_phi_phi0 + d_x_theta*m_phi_theta + d_x_phi*m_phi_phi  )   ;
    
  xpCov(2,3) =  d_pz_theta* (  d_y_d0*m_theta_d0 + d_y_phi0*m_theta_phi0 + d_y_theta*m_theta_theta + d_y_phi*m_phi_theta  )   + 
    d_pz_phi* (  d_y_d0*m_phi_d0 + d_y_phi0*m_phi_phi0 + d_y_theta*m_phi_theta + d_y_phi*m_phi_phi  )   ;
    
  xpCov(3,3) =  d_pz_theta* (  d_z_d0*m_theta_d0 + d_z_phi0*m_theta_phi0 + d_z_theta*m_theta_theta + d_z_phi*m_phi_theta  )   + 
    d_pz_phi* (  d_z_d0*m_phi_d0 + d_z_phi0*m_phi_phi0 + d_z_theta*m_phi_theta + d_z_phi*m_phi_phi  )   ;
    

  ppCov(1,1) = d_px_theta* ( d_px_theta*m_theta_theta + d_px_phi*m_phi_theta  )   + d_px_phi* (  d_px_theta*m_phi_theta + d_px_phi*m_phi_phi  );
   
  ppCov(2,1) = d_py_theta* ( d_px_theta*m_theta_theta + d_px_phi*m_phi_theta  )   + d_py_phi* (  d_px_theta*m_phi_theta + d_px_phi*m_phi_phi  );

  ppCov(2,2) = d_py_theta* ( d_py_theta*m_theta_theta + d_py_phi*m_phi_theta  )   + d_py_phi* (  d_py_theta*m_phi_theta + d_py_phi*m_phi_phi  );
    
  ppCov(3,1) = d_pz_theta* ( d_px_theta*m_theta_theta + d_px_phi*m_phi_theta  )   + d_pz_phi* (  d_px_theta*m_phi_theta + d_px_phi*m_phi_phi  );
     
  ppCov(3,2) = d_pz_theta* ( d_py_theta*m_theta_theta + d_py_phi*m_phi_theta  )   + d_pz_phi* (  d_py_theta*m_phi_theta + d_py_phi*m_phi_phi  );
  
  ppCov(3,3) = d_pz_theta* ( d_pz_theta*m_theta_theta + d_pz_phi*m_phi_theta  )   + d_pz_phi* (  d_pz_theta*m_phi_theta + d_pz_phi*m_phi_phi  );
...
//------------------------------------------------------------------------
void
CosmicTrkMomCalc::calcCurvAllWeights(const TrkSimpTraj& theTraj,
				     const BField& theField,
				     double fltlen,
				     HepVector& pos,
				     HepVector& mom,
...
//------------------------------------------------------------------------
void
CosmicTrkMomCalc::calcCurvAllWeightsOLD(const TrkSimpTraj& theTraj,
				     const BField& theField,
				     double fltlen,
				     HepVector& pos,
				     HepVector& mom,
...
//------------------------------------------------------------------------
void
CosmicTrkMomCalc::calcNeutAllWeights(const TrkSimpTraj& theTraj,
				     const BField& theField,
				     double fltlen,
				     HepVector& pos,
				     HepVector& mom,
