...
/*******************************************************************/
  ...
  ptr_t CalCalibMaker::fromDb(CalEnergyCalib::cptr_t ecalib, CalTimeCalib::cptr_t tcalib) { //TODO - input from time table too
  ...
  for (auto const& rowe : ecalib->rows()) {
    for (auto const& rowt : tcalib->rows()) {
      cvec[rowe.roid().id()] = CalCalibPar(rowe.ADC2MeV(),rowt.T0()); 
    }
  }
