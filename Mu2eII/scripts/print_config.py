...
#
# call: Mu2eII/scripts/gen_fcl.py --project=Mu2eII --dsid=bmum3 
#   project  : project name
#   dsid     : input dataset family
#-------------------------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
    def print_config(self):
        ...
        line += '--------------------------------------'
        print(line)
        print('stage       job                 input DSID  N(seg) N(outputs)  output DSID',end='');
        print('      outputFnPattern                base FCL');
        print(line)
        for k in self.fConfig.fStage.keys():
            ...
            for j in s.fJob.keys():
                ...
                print('%-5s %-25s %10s %6i %7i'%(s.name(),job.name(),job.input_dsid(),job.n_segments(),nos),end=' ')
                print('      %10s %20s %20s'%(job.output_dsid(0),job.output_fn_pattern(0),job.base_fcl()));
                if (nos > 1):
                    for os in range(1,nos):
                        print('%62s  %10s %20s'%('',job.output_dsid(os),job.output_fn_pattern(os)));
            print(line);
        return 0;
