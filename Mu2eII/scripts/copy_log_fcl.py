#!/usr/bin/env python
#------------------------------------------------------------------------------
# call: Mu2eII/scripts/copy_log_fcl.py --project=Mu2eII --grid_id=35469055[,xxxxxx,[yyy]]
...
#------------------------------------------------------------------------------
    def copy_log_fcl(self,job):
        name = 'list_pnfs_files'

        topdir     = job.grid_output_dir();
        print('topdir:',topdir)

        odir  = '/mu2e/data/users/mu2epro/'+self.fProject+'/'+job.input_dsid()+'.'+job.stage()+'_'+job.name()
...
if (__name__ == '__main__'):
    ...
    for item in x.fGridIDList:
        ...
        if (x.fUseRunningDir == 0): fn = x.fCompletedDir+'/'+grid_id;
        else                      : fn = x.fRunningDir  +'/'+grid_id;
        ...
        if (doit): 
            x.copy_log_fcl(job)
            #------------------------------------------------------------------------------
            # end: either update job status file with bit 2 [log files copied] added, [default]
            #      or recreate it in a new directory
            #------------------------------------------------------------------------------
            if ((self.fUseRunningDir == 1):
                fn_new = self.fCompletedDir+'/'+job.id()+'.tmp'
                rc     = self.fGridJob.write_status_file(fn_new);
                if (rc == 0): os.replace(fn_new,fn);
            else:         
                fn_new = self.fCompletedDir+'/'+str(self.fGridJob.id());
                rc     = self.fGridJob.write_status_file(fn_new);
            ...
            rc = job.write_status_file(new_fn);
            if (rc == 0):
                os.replace(new_fn,old_fn);
