...
#
# call: Mu2eII/scripts/gen_fcl.py --project=Mu2eII --dsid=bmum3 --stage=s2  --job=sim [--recover=grid_id] [--fileset=xxxx]
...
#------------------------------------------------------------------------------
    def ParseParameters(self):
        ...
        if (self.fRecover):
            ...
            fn              = 'tmp/Mu2eII/grid_job_status/'+self.fGridID;
...
#------------------------------------------------------------------------------
    def make_fcl_tarball(self,fcldir,tarball):
        ...
        # cmd     = ['Mu2eII/scripts/make_fcl_tarball',fcldir,tarball]
...
#------------------------------------------------------------------------------
        ...
        for i in range(0,nfiles):
            ...
            for line in f.readlines():
                ...
                if ('physics.filters.InitStntuple.histFileName' in line):
                    line   = 'physics.filters.InitStntuple.histFileName : \"'+stn_fn+'"\n'
                elif ('MU2EGRIDDSOWNER' in line):
                    line.replace('MU2EGRIDDSOWNER',self.fOwner)
    ...
    def gen_fcl(self,stage,job):
        ...
        if (ids.defname() != 'generator'):
            ...
            if (self.fFileset):
                if (not self.fFirstSubrun):
                    # fileset is defined, first subrun - is not
        ...
        if (job.fAuxInputs):
            ...
            for key in job.fAuxInputs.keys():
                ...
                filelist = 'Mu2eII/'+dataset.dsid_stub()+'/catalog/'+dataset.defname()+'.files'
