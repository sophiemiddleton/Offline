...
#------------------------------------------------------------------------------        
        ...
        job.fRunNumber               = 1000
        job.fBaseFcl                 = project+'/'+familyID+'/'+s.name()+'_endpoint_photons_gen_sim_dig_'+familyID+'.fcl'

        job.fInputDataset            = Dataset('generator', 'rmce4s00b0', 'local');
...
#------------------------------------------------------------------------------        
        ...
        job.fRunNumber               = 1000
        job.fBaseFcl                 = project+'/'+familyID+'/'+s.name()+'_endpoint_photons_gen_sim_dig_b1_'+familyID+'.fcl'

        job.fInputDataset            = Dataset('generator', 'rmce4s00b1', 'local');
        job.fNInputFiles             = 100
        define_mixing_inputs(job);

        job.fMaxInputFilesPerSegment =  1
        job.fNEventsPerSegment       =  300000 #~0.03 s/event on profile build, ~0.2% survive filtering
        job.fResample                = 'no'                         # yes/no
...
#------------------------------------------------------------------------------        
        ...
        defname                      = 'dig.mu2e.rmce4s41b0.Mu2eII.art'
...
#------------------------------------------------------------------------------
        ...
        defname                      = 'dig.mu2e.rmce4s41b1.Mu2eII.art'  # files in PNFS, not on tape yet
