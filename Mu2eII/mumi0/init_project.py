...
#------------------------------------------------------------------------------
        ...
        job.fInputDataset            = Dataset('generator','mumi0s00b0','local');
...
#------------------------------------------------------------------------------
        ...
        job.fInputDataset            = Dataset('generator','mumi0s00b1','local');
...
#------------------------------------------------------------------------------
# s4:sim_b2 : 1-batch mode electron dataset, no concatenation needed, 1M = 500 (segments)x2000(events per segment)
#------------------------------------------------------------------------------
        ...
        job.fInputDataset            = Dataset('generator','mumi0s00b2','local');
        job.fNInputFiles             = 500                      # number of the job segments for 1M events total
...
#------------------------------------------------------------------------------
        ...
        dsn                          = 'dig.mu2e.mumi0s41b0.Mu2eII.art'   # dataset: mu2e.mumi0s41b0.Mu2eII.art
        job.fInputDataset            = Dataset(dsn,'','local')            # dataset: mu2e.mumi0s41b0.Mu2eII.art
...
#------------------------------------------------------------------------------
        ...
        defname                      = 'dig.mu2e.mumi0s41b1.Mu2eII.art'  # files in PNFS, not on tape yet
        ...
        job.fDescription             = desc;
#------------------------------------------------------------------------------
# s5:reco_stn_b2  : reconstruct and stntuple 2-batch mode dataset, don't write output, only stntuple's
#------------------------------------------------------------------------------
        job                          = s.new_job('reco_stn_b2');
        job.fRunNumber               = 1000;
        job.fBaseFcl                 = project+'/'+familyID+'/'+s.name()+'_reco_stn_b1_'+familyID+'.fcl'

        defname                      = 'dig.mu2e.mumi0s41b2.Mu2eII.art'  # files in PNFS, not on tape yet
        job.fInputDataset            = Dataset(defname,'','local')       # dataset: mu2e.cele0s41b0.Mu2eII.art
        job.fNInputFiles             = -1                                # placeholder, the real number is defined by the input dataset
        ...
        job.fDescription             = desc;
#------------------------------------------------------------------------------
# s6:reco_stn  : remake s5, produce STNTUPLES
#------------------------------------------------------------------------------
        s                            = self.new_stage('s6');
        job                          = s.new_job('reco_stn');
        ...
        dsn                          = 'mcs.mu2e.mumi0s51b0.Mu2eII.art'  # dataset: mu2e.mumi0s41b0.Mu2eII.art
        job.fInputDataset            = Dataset(dsn,'','sam')             # dataset: mu2e.mumi0s41b0.Mu2eII.art
        job.fNInputFiles             = -1                                # placeholder, the real number is defined by the input dataset
