...
#------------------------------------------------------------------------------
        ...
        job.fInputDataset            = Dataset('generator','cele0s00b0','local');
...
#------------------------------------------------------------------------------
        ...
        job.fInputDataset            = Dataset('generator','cele0s00b1','local');
...
#------------------------------------------------------------------------------
        ...
        job.fInputDataset            = Dataset('generator','cele0s00b2','local');
        job.fNInputFiles             = 1000                      # number of the job segments for 1M events total
        define_mixing_inputs(job);

        job.fMaxInputFilesPerSegment =  1                       # MC generator
        job.fNEventsPerSegment       =  1000                    # for 1-batch mode signal+minbias events generate 2000 events/segment
        job.fResample                = 'no'                     # yes/no
        job.fMaxMemory               = '3000MB'
        job.fRequestedTime           = '12h'
        job.fIfdh                    = 'xrootd'                 # ifdh/xrootd
...
#------------------------------------------------------------------------------
        ...
        dsn                          = 'dig.mu2e.cele0s41b0.Mu2eII.art'   # dataset: mu2e.cele0s41b0.Mu2eII.art
        job.fInputDataset            = Dataset(dsn,'cele0s41b0','local')  # dataset: mu2e.cele0s41b0.Mu2eII.art
...
#------------------------------------------------------------------------------
        ...
        defname                      = 'dig.mu2e.cele0s41b0.Mu2eII.art'   # dataset: mu2e.cele0s41b0.Mu2eII.art
...
#------------------------------------------------------------------------------
        ...
        defname                      = 'dig.mu2e.cele0s41b1.Mu2eII.art'  # files in PNFS, not on tape yet
        job.fInputDataset            = Dataset(defname,'','local')       # dataset: mu2e.cele0s41b0.Mu2eII.art
        job.fNInputFiles             = -1                                # placeholder, the real number is defined by the input dataset
...
#------------------------------------------------------------------------------
        ...
        defname                      = 'dig.mu2e.cele0s41b2.Mu2eII.art'  # files in PNFS, not on tape yet
        job.fInputDataset            = Dataset(defname,'','local')       # dataset: mu2e.cele0s41b0.Mu2eII.art
        job.fNInputFiles             = -1                                # placeholder, the real number is defined by the input dataset
        ...
        job.fDescription             = desc;
#------------------------------------------------------------------------------
# s6:reco_stn_b1  : remaking S5 b1 ntuples
#------------------------------------------------------------------------------
        s                            = self.new_stage('s6');
        ...
        job.fDescription             = desc;
#------------------------------------------------------------------------------
# s6:reco_stn_b2  : remaking S5 B2 ntuples starting from digis
#------------------------------------------------------------------------------
        job                          = s.new_job('reco_stn_b2');
        job.fRunNumber               = 1000;
        job.fBaseFcl                 = project+'/'+familyID+'/'+s.name()+'_reco_stn_b2_'+familyID+'.fcl'
        ...
        defname                      = 'dig.mu2e.cele0s41b2.Mu2eII.art'  # files in PNFS, not on tape yet
        job.fInputDataset            = Dataset(defname,'','local')       # dataset: mu2e.cele0s41b2.Mu2eII.art
        job.fNInputFiles             = -1                                # placeholder, the real number is defined by the input dataset
        ...
        job.fDescription             = desc;
#------------------------------------------------------------------------------
# s7:reco_stn_b2  : S6B2 ntupls had B1 calorimeter noise, remake starting from cele0s41b2
#------------------------------------------------------------------------------
        s                            = self.new_stage('s7');
        ...
        defname                      = 'dig.mu2e.cele0s41b2.Mu2eII.art'  # files in PNFS, not on tape yet
        job.fInputDataset            = Dataset(defname,'','sam')         # dataset: mu2e.cele0s41b2.Mu2eII.art
        job.fNInputFiles             = -1                                # placeholder, the real number is defined by the input dataset
        ...
        job.fDescription             = desc;
#------------------------------------------------------------------------------
# s8:reco_stn_b1  : making b1 ntuples with 'cosmic reco path'
#------------------------------------------------------------------------------
        s                            = self.new_stage('s8');
