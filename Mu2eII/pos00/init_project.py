...
#------------------------------------------------------------------------------
    def __init__(self):
        ...
        job.fInputDsID               = 'pos00s00b0'        # muon stops
        ...
        job.fInputDsID               = 'pos00s41b0'                       # concatenation
        dsn                          = project+'.'+job.fInputDsID+'.art'  # dataset: mu2e.pos00s41b0.Mu2eII.art
        job.fInputDataset            = Dataset(dsn,'pos00s41b0','local')  # dataset: mu2e.pos00s41b0.Mu2eII.art
        ...
        job.fInputDsID               = 'pos00s41b0'                      # dsID
        dsn                          = project+'.'+job.fInputDsID+'.art' # dataset: mu2e.pos00s41b0.Mu2eII.art
        job.fInputDataset            = Dataset(dsn,'pos00s41b0','local') # dataset: mu2e.pos00s41b0.Mu2eII.art
        ...
        job.fMaxMemory               = '2000MB'
        job.fRequestedTime           = '12h'
        job.fIfdh                    = 'ifdh'                            # ifdh/xrootd, 1 input file, IFDH is more robust now
        job.fOutputPath              = [ 'out' ]
        job.fOutputStream            = [ 'defaultOutput'                ]
        job.fOutputDsID              = [ dsid+s.name()+'1b0'            ]
        job.fOutputFnPattern         = [ 'dig.mu2e.'+job.fOutputDsID[0] ]
