#ifndef __fit_cb2_asymm_C__
#define __fit_cb2_asymm_C__
///////////////////////////////////////////////////////////////////////////////
...
public:
  ...
  cb2_asymm(const char* Name = "x");
  ~cb2_asymm();

  static double  gauss(double* X, double* P);
  static double  f_cb2_asymm(double* X, double* P);    // double Crystall Ball itself
...
//-----------------------------------------------------------------------------
cb2_asymm::cb2_asymm(const char* Name): TNamed(Name,Name) {
  fHist  = nullptr;
  fHGaus = nullptr;
  fFunc  = nullptr;
}
...
//-----------------------------------------------------------------------------
double cb2_asymm::gauss(double* X, double* P) {
  double x0, f(0), sig1,sig2, xdx;
...
//-----------------------------------------------------------------------------
double cb2_asymm::f_cb2_asymm(double* X, double* P) {
  double x0, x1, f, sig1, sig2, alpha1, abs_alpha1, alpha2, abs_alpha2, n1, n2, a, b, dx0, dx1, xdx;
...
//-----------------------------------------------------------------------------
void cb2_asymm::init_parameters(TF1* F, 
			  double P0, double P1, double P2, double P3, 
			  double P4, double P5, double P6, double P7) {
  F->SetParameter(0,P0);
  F->SetParameter(1,P1);
...
//-----------------------------------------------------------------------------
void cb2_asymm::fit_histogram(TH1F* Hist, TF1* F, double XMin, double XMax) {
  Hist->GetXaxis()->SetRangeUser(XMin,XMax);
  Hist->Fit(F,"L","",XMin,XMax);
}

//-----------------------------------------------------------------------------
void cb2_asymm::create_fit_function(TF1*& F, double X0, double XMin, double XMax) {

  fFunc = new TF1(Form("%s_f_cb2_asymm",GetName()),cb2_asymm::f_cb2_asymm,XMin,XMax,8);

  F->SetParName(0,"anorm");
  F->SetParName(1,"mean");
  F->SetParName(2,"sigma1");
  F->SetParName(3,"alpha1");
  F->SetParName(4,"N1");
  F->SetParName(5,"alpha2");
  F->SetParName(6,"N2");
  F->SetParName(7,"sigma2");
...
//-----------------------------------------------------------------------------
void cb2_asymm::fit(TH1* Hist, double X0, double XMin, double XMax, double Sigma = -1.) {
  ...
  init_parameters    (fFunc,anorm,X0,0.180,1,.4,1.,4,0.240);
...
//   fFunc->Draw("same");
  ...
  fGaus = new TF1(Form("%s_f_gaus",GetName()),cb2_asymm::gauss,XMin,XMax,4);
  ...
  fGaus->SetParameter(3,fFunc->GetParameter(7));
...
//-----------------------------------------------------------------------------
  ...
  double sigma1 = fFunc->GetParameter("sigma1" );
  double alpha2 = fFunc->GetParameter("alpha2");
  double x1     = x0+fabs(sigma1*alpha2);
  double sigma2 = fFunc->GetParameter("sigma2");

  printf("x0 = %10.4f, sigma1 = %10.4f alpha2 = %10.4f\n",x0,sigma1, alpha2);
 
  double htail = (fFunc->Integral(x1,XMax)-fGaus->Integral(x1,XMax))/(Hist->GetBinWidth(1));
  double fractail=(fFunc->Integral(x1,XMax)-fGaus->Integral(x1,XMax))/(fFunc->Integral(x1,XMax));

  printf("total = %12.5f, HTail = %12.5f, htail/total : %12.5e\n, fractail=%12.5f\n",total,htail,htail/total,fractail);
}

//-----------------------------------------------------------------------------
void cb2_asymm::fit(const char* File, const char* Module, const char* Hist, 
	      double X0, double XMin, double XMax, double Sigma = -1.) {
