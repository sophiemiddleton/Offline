...
#------------------------------------------------------------------------------        
        ...
        job                          = s.new_job('sim');
        job.fBaseFcl                 = project+'/'+dsid+'/'+s.name()+'_endpoint_photons_gen_sim_dig_'+dsid+'.fcl'
        job.fRunNumber               = 1000
        job.fInputStage              = 's0'

        job.fInputDataset            = Dataset('generator', 'rmce6s00b0', 'local'); # generator
        job.fNInputFiles             =  1000    
...
#------------------------------------------------------------------------------        
        ...
        job.fInputDsID               = 'rmce6s41b0'        # conversions
        dsn                          = 'dig.mu2e.'+job.fInputDsID+'.Mu2eII.art' # dataset: dig.mu2e.rmce6s41b0.Mu2eII.art
        job.fInputDataset            = Dataset(dsn,'rmce6s41b0','local')      
        job.fNInputFiles             = -1                 
