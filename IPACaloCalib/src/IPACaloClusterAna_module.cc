...
namespace mu2e {
  ...
  class IPACaloClusterAna : public art::EDAnalyzer {
     public:
	struct Config {
		...
		explicit IPACaloClusterAna(const Parameters& conf);
		virtual ~IPACaloClusterAna() {}
   ...
   IPACaloClusterAna::IPACaloClusterAna(const Parameters& conf):
		art::EDAnalyzer(conf),
  ...
  void IPACaloClusterAna::beginJob(){
  ...
  void IPACaloClusterAna::analyze(const art::Event& event) {
...
bool IPACaloClusterAna::findData(const art::Event& evt){
 ...
 void IPACaloClusterAna::endJob(){} 

}

DEFINE_ART_MODULE(mu2e::IPACaloClusterAna);
