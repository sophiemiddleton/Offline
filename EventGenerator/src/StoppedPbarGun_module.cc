// cloned from StoppedParticleReactionGun by Andrei Gaponenko, 2013
...
namespace mu2e {
  ...
  //================================================================
  class StoppedPbarGun : public art::EDProducer {
    ...
    RootTreeSampler<IO::StoppedPbar> stops_;
//-----------------------------------------------------------------------------
// histogramming
//-----------------------------------------------------------------------------
    bool    doHistograms_;
  ...
  public:
    explicit StoppedPbarGun(const fhicl::ParameterSet& pset);

    virtual void produce(art::Event& event);
  };

  //================================================================
  StoppedPbarGun::StoppedPbarGun(const fhicl::ParameterSet& pset)
    : EDProducer{pset}
    ...
    , stops_(eng_, pset.get<fhicl::ParameterSet>("pbarStops"))
    , doHistograms_       (pset.get<bool>("doHistograms",true ) )
  {
    produces<mu2e::GenParticleCollection>();

    if(genId_ == GenId::enum_type::unknown) {
      throw cet::exception("BADCONFIG")<<"StoppedPbarGun: unknown genId "
                                       <<psphys_.get<std::string>("genId", "StoppedPbarGun")
                                       <<"\n";
    }

    if (verbosityLevel_ > 0) {
      std::cout<<"StoppedPbarGun: using = "
               <<stops_.numRecords()
               <<" stopped particles"
               <<std::endl;

      std::cout<<"StoppedPbarGun: using GenId = " << genId_ << std::endl;

      std::cout<<"StoppedPbarGun: producing particle "<< pdgId_ << ", mass = "<< mass_ << std::endl;

      std::cout <<"StoppedPbarGun: spectrum shape = "
    ...
    if (verbosityLevel_ > 1){
      std::cout <<"StoppedPbarGun: spectrum: " << std::endl;
      spectrum_.print();
    }

    if ( doHistograms_ ) {
      art::ServiceHandle<art::TFileService> tfs;
      //      art::TFileDirectory tfdir = tfs->mkdir( "StoppedPbarGun");
      _hEnergy = tfs->make<TH1F>("hEnergy", "Energy"      , 2400,   0.0,  120);
      _hMom    = tfs->make<TH1F>("hMom"   , "Momentum"    ,  501,  -0.5,  250);
  ...
  //================================================================
  StoppedPbarGun::SpectrumVar StoppedPbarGun::parseSpectrumVar(const std::string& name) {
    if (name == "totalEnergy"  )  return TOTAL_ENERGY;
    if (name == "kineticEnergy")  return KINETIC_ENERY;
    if (name == "momentum"     )  return MOMENTUM;
    throw cet::exception("BADCONFIG")<<"StoppedPbarGun: unknown spectrum variable "<<name<<"\n";
  }


  //================================================================
  void StoppedPbarGun::produce(art::Event& event) {
...
// energy
//-----------------------------------------------------------------------------
  double StoppedPbarGun::generateEnergy() {
    double res = spectrum_.sample(randSpectrum_.fire());

    if (res < 0.0) {
      throw cet::exception("BADE")<<"StoppedPbarGun: negative energy "<< res <<"\n";
    }
...
DEFINE_ART_MODULE(mu2e::StoppedPbarGun);
