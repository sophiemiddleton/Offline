...
namespace mu2e {
        ...
        , id(p->pdgId())
      {
        ...
        if(!p->endDefined()) {
          throw cet::exception("BADINPUTS")
            <<"MomAnalyzer: input SimParticle does not have end defined!\n";
        }
      }
    };
  ...
  //================================================================
  class MomAnalyzer : public art::EDAnalyzer {
  public:
    ...
    explicit MomAnalyzer(const Parameters& conf);
  ...
  //================================================================
  MomAnalyzer::MomAnalyzer(const Parameters& conf) :
    art::EDAnalyzer(conf),
  ...
  //================================================================
  void MomAnalyzer::analyze(const art::Event& event) {
  ...
  //================================================================
  void MomAnalyzer::process(const art::Ptr<SimParticle>& p, const VspMC& spMCColls) {
  ...
  //================================================================
  bool MomAnalyzer::is_leave(const SimParticle& p) {
...
DEFINE_ART_MODULE(mu2e::MomAnalyzer);
