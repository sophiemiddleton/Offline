...
namespace mu2e {
  ...
  class MyReadAntiProtonSteps : public art::EDAnalyzer {
  public:
    ...
    explicit MyReadAntiProtonSteps(fhicl::ParameterSet const& pset) :
      art::EDAnalyzer(pset),
      ...
      _diagLevel(pset.get<int>("diagLevel",0))
    {
      ...
      if( pdg_ids.size()>0 ) {
        cout << "MyReadAntiProtonSteps: save following particle types in the ntuple: ";
  ...
  void MyReadAntiProtonSteps::beginJob(){
  ...
  void MyReadAntiProtonSteps::beginRun(art::Run const& run){

  }

  void MyReadAntiProtonSteps::analyze(const art::Event& event) {
...
using mu2e::MyReadAntiProtonSteps;
DEFINE_ART_MODULE(MyReadAntiProtonSteps);
