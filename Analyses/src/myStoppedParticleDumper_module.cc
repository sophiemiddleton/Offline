...
namespace mu2e {
  ...
  namespace {
    ...
    struct StopInfo {
        ...
        , pdg(abs(p->pdgId()))
      {
        ...
        /*std::cout<<"pdg"<<abs(p->pdgId())<<std::endl;
        double pdg = 211;
        ...
        if(abs(p->pdgId()) == 211 and t > 400) nEvents +=1;
  ...
  void myStoppedParticleDumper::beginJob() {
    ...
    std::string branchDesc("x/F:y/F:z/F:time/F:pt/F:pdg/I");
    if(writeProperTime_) {
  ...
  void myStoppedParticleDumper::process(const art::Ptr<SimParticle>& p, const VspMC& spMCColls) {
    const float tau = 0;//SimParticleGetTau::calculate(p,spMCColls,decayOffCodes_) ;//: -1;
    //const PhysicsParams& gc = *GlobalConstantsHandle<PhysicsParams>();
    //const float tau = p->endProperTime() / gc.getParticleLifetime(p->pdgId());
    if(abs(p->pdgId()) == 211){// and p->endGlobalTime()> 400) {
