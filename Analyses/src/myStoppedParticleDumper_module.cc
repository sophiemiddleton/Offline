...
//
// Andrei Gaponenko, 2013, 2015, Editted by S. Middleton 2020
...
namespace mu2e {
  ...
  namespace {
    ...
    struct StopInfo {
      ...
      StopInfo() : x(), y(), z(), px(), py(), pz(), pt(), t(), tau(), code(), id(), productionP(), productionCosTheta() {}
      ...
      StopInfo(const art::Ptr<SimParticle>& p, const VspMC& spMCcolls, float tt)
        ...
        , code(p->stoppingCode())
        , id(p->pdgId())
        , productionP(sqrt(p->startMomentum().x()*p->startMomentum().x()+p->startMomentum().y()*p->startMomentum().y()+p->startMomentum().z()*p->startMomentum().z()))
			  , productionCosTheta(p->startMomentum().z()/sqrt(p->startMomentum().x()*p->startMomentum().x()+p->startMomentum().y()*p->startMomentum().y()+p->startMomentum().z()*p->startMomentum().z()))
      {
  ...
  private:
    ...
    TTree *nt_;
    StopInfo data_;
    Float_t productionP, productionCosTheta;
    TH2F *pvz;
    TH2F *tvz;
    TH1F *weightedP;
    TH1F *weightedZ;
  ...
  void StoppedParticlesDumper::beginJob() {
    ...
    std::string branchDesc("x/F:y/F:z/F:px/F:py/F:pz/F:pt/F:time/F:code/I:id/I:productionP/F:productionCosTheta/F");
    if(writeProperTime_) {
      branchDesc += ":tauNormalized/F";
    }
    nt_ = tfs->make<TTree>( "Pions", "Particles ntuple");
    nt_->Branch("Pions", &data_, branchDesc.c_str());
    nt_->Branch("ProductionMomentum", &productionP, "productionP/F");
    nt_->Branch("ProductionCosTheta", &productionCosTheta, "productionCosTheta/F");
  ...
  void StoppedParticlesDumper::analyze(const art::Event& event) {
    ...
    for ( const auto& iColl : hitColls_ ){
         ...
         //for(const auto& p : *spColl) {
         //    const StepPointMC hit = p;
         //    std::cout<<" In " << hit.volumeId()<<std::endl;
            //if (hit.volumeId() == VirtualDetectorId::PSPbarIn) { n+=1; std::cout<<n<<" In " << hit.volumeId()<<std::endl;}
  ...
  void StoppedParticlesDumper::process(const art::Ptr<SimParticle>& p, const VspMC& spMCColls) {
     ...
     //----pions
			 ...
			 productionP = sqrt(p->startMomentum().x()*p->startMomentum().x()+p->startMomentum().y()*p->startMomentum().y()+p->startMomentum().z()*p->startMomentum().z());
			 productionCosTheta = p->startMomentum().z()/productionP;
    ...
    weightedP->Fill(sqrt(p->startMomentum().x()*p->startMomentum().x()+p->startMomentum().y()*p->startMomentum().y()+p->startMomentum().z()*p->startMomentum().z()), weight);
